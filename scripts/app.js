...
const generatePassword = (length) => {
    ...
    //resets each time function runs
    let possiblePasswordChars = []

    if(includeDigits){
        digits.forEach(digit => {
            possiblePasswordChars.push(digit)
        })
    }

    if(specialChar){
        specials.forEach(special => {
            possiblePasswordChars.push(special)
        })
    }

    if(letterMix){
        lettersLowerCase.forEach(letter => {
            possiblePasswordChars.push(letter);
        })
...
copyPasswordBtn.addEventListener('click', () => {
    ...
    confimation.classList.add('active');
    setTimeout(() => {
        confimation.classList.remove('active');
    }, 2000)
})